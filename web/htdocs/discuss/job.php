...
if ($action=='delimg'){
	...
	*******/
	...
	$S_sql = $_POST['step']=='3' ? ",t.subject,t.postdate,t.rewardinfo" : '';

	$rt=$db->get_one("SELECT p.tid,p.fid,p.tid,p.author as pauthor,p.authorid as pauthorid,p.ifreward,t.author as tauthor,t.authorid as tauthorid $S_sql FROM pw_posts p LEFT JOIN pw_threads t ON p.tid=t.tid WHERE p.pid='$pid' AND t.ifreward='1'");
	!$rt && Showmsg('illegal_tid');
	$rt['tauthorid']!=$winduid && Showmsg('reward_noright');
	if(!$step){
		require_once(R_P.'require/header.php');
		require_once PrintEot('reward');footer();
	}elseif($_POST['step']=='3'){
		list($rw_b_name,$rw_b_val,$rw_a_name,$rw_a_val)=explode('|',$rt['rewardinfo']);
		if($type=='1'){
			$info_add="\n".$rt['pauthor'].'|'.$pid;
			$db->update("UPDATE pw_threads SET ifreward='2',rewardinfo=CONCAT(rewardinfo,'$info_add') WHERE tid='$rt[tid]'");
			$db->update("UPDATE pw_posts SET ifreward='2' WHERE pid='$pid'");
			addcredit($rt['pauthorid'],$rw_b_name,$rw_b_val);  /*最佳答案者加分*/
			...
			if($ifreward==1){
				$rew=$db->get_one("SELECT pid FROM pw_posts WHERE tid='$rt[tid]' AND ifreward='1' AND authorid='$rt[pauthorid]' LIMIT 1");
				$rew && Showmsg('reward_have');
			}else{
				$db->update("UPDATE pw_posts SET ifreward='1' WHERE tid='$rt[tid]' AND ifreward='0' AND pid>'$pid' AND authorid='$rt[pauthorid]' ORDER BY postdate ASC LIMIT 1");
			}
	...
	*******/
	...
	$S_sql='';
	if($step=='3'){
		$S_sql=',t.rewardinfo,t.postdate,f.allowreward';
		$ifmsg && $S_sql.=',t.fid,t.subject';
	}	
	$rt=$db->get_one("SELECT t.author,t.authorid,f.forumadmin,f.fupadmin $S_sql FROM pw_threads t LEFT JOIN pw_forums f USING(fid) WHERE t.tid='$tid' AND t.ifreward='1' AND f.allowreward>0");
	!$rt && Showmsg('illegal_tid');
	...
	if(!$step){
		...
		if($type=='1'){
			list($rw_b_name,$rw_b_val,$rw_a_name,$rw_a_val)=explode('|',$rt['rewardinfo']);
			$info_add="\n".$lang['cancle'];
			$db->update("UPDATE pw_threads SET ifreward='2',rewardinfo=CONCAT(rewardinfo,'$info_add') WHERE tid='$tid'");
			$rw_b_val*=2;
	...
	!$winduid && Showmsg('undefined_action');
	$rt=$db->get_one("SELECT t.fid,t.subject,t.authorid,t.postdate,f.forumadmin FROM pw_threads t LEFT JOIN pw_forums f USING(fid) WHERE t.tid='$tid' AND t.ifreward='1'");
	!$rt && Showmsg('illegal_tid');
