...
if(!$step){
	...
	if ($schedid){
		...
		while($sch = $db->fetch_array($query)){
			...
			if ($sch['pollid']&&$sch['locked'] == 0){
				$sch['status'] = "<img src='$imgpath/$stylepath/thread/vote.gif' border=0>";
			} elseif ($sch['pollid']&&$sch['locked']>0){
				$sch['status'] = "<img src='$imgpath/$stylepath/thread/votelock.gif' border=0>";
			} else{
				if ($sch['locked'] == 1){
					$sch['status'] =  "<img src='$imgpath/$stylepath/thread/topiclock.gif' border=0>";
				} elseif ($sch['locked'] == 2){
					$sch['status'] = "<img src='$imgpath/$stylepath/thread/topicclose.gif' border=0>";
				} elseif ($sch['replies']>=10){
					$sch['status'] = "<img src='$imgpath/$stylepath/thread/topichot.gif' border=0>";
				} else{
					$sch['status'] = "<img src='$imgpath/$stylepath/thread/topicnew.gif' border=0>";
				}
			}
