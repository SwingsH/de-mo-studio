...
if(!$action){
	if($_POST['job']!="submit"){
		...
		$get_style.=getstyles($db_defaultstyle);
		$get_style.="</select>";
		...
		if ($job=="submit"){
			...
			if(file_exists(R_P."data/style/$stylefile.php")){
				if(P_unlink(R_P."data/style/$stylefile.php")){
	...
	if ($_POST['job']!="submit"){
		include_once Pcv(R_P."data/style/$stylefile.php");
		include PrintEot('setstyles');exit;
	} elseif ($job=="submit") {
