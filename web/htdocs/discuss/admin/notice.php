...
if(@filemtime(D_P.'data/bbscache/file_lock.txt')<$timestamp-3600*24){
	$wget=PostHost("http://nt.phpwind.com/src/pw5/union1.php?url=$_SERVER[HTTP_HOST]$_SERVER[PHP_SELF]&repair=$wind_repair&charset=$db_charset","");
	...
	$wget=PostHost("http://nt.phpwind.com/src/pw5/info1.php?charset=$db_charset","");
	$wget=preg_replace("/(.+)(\<pwret\>)(.+)?(\<\/pwret\>)(.*?)/is","\\3",$wget);
	writeover(D_P.'data/bbscache/info.txt',$wget);
}elseif(@filemtime(D_P.'data/bbscache/userpay.txt')<$timestamp-3600*24){
	$wget=PostHost("http://nt.phpwind.com/src/pw5/userpay1.php?charset=$db_charset","");
	$wget=preg_replace("/(.+)(\<pwret\>)(.+)?(\<\/pwret\>)(.*?)/is","\\3",$wget);
