...
if($a_type=='article'){
	...
	if(empty($action)){
		...
		if(empty($_POST['step'])){
			...
			$pages = numofpage($rt['sum'],$page,ceil($rt['sum']/$nums),"$admin_file?adminjob=superdel&a_type=article&action=$action&fid=$fid&tid=$tid&pstart=$pstart&pend=$pend&author=".rawurlencode($author)."&keyword=".rawurlencode($keyword)."&userip=$userip&tcounts=$tcounts&counts=$counts&nums=$nums&");
		...
		if($_POST['step']==2 || $_POST['direct']){
			...
			* 删除帖子
			*/
			$query=$db->query("SELECT tid,fid,postdate,ifupload,pollid FROM pw_threads WHERE tid IN($dtids)");
			while(@extract($db->fetch_array($query))){
				$htmurl=$htmdir.'/'.$fid.'/'.date('ym',$postdate).'/'.$tid.'.html';
				if(file_exists(R_P.$htmurl)){
					P_unlink(R_P.$htmurl);
