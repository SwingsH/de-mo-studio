...
if(!$sid){
	$sid = randstr(8);
	Cookie('sid',$sid);
	$db->update("REPLACE INTO pw_cknum SET sid='$sid',nmsg='$nmsg',time='$timestamp'");
}else{
	$newsid = randstr(8);
	Cookie('sid',$newsid);
	$db->update("UPDATE pw_cknum SET sid='$newsid',nmsg='$nmsg',time='$timestamp' WHERE sid='$sid'");
	if(!$db->affected_rows()){
		$db->update("REPLACE INTO pw_cknum SET sid='$newsid',nmsg='$nmsg',time='$timestamp'");
	}
}
