...
if(!$action){
	...
	while($rt = $db->fetch_array($query)){
		...
		if(!$rt['ifcheck'] || strpos($rt['content'],"[post]") !== false && strpos($rt['content'],"[/post]") !== false || strpos($rt['content'],"[hide") !== false && strpos($rt['content'],"[/hide]") !== false || strpos($rt['content'],"[sell") !== false && strpos($rt['content'],"[/sell]") !== false){
			$rt['a_url']="none";
		}
	...
	if($rt){
		$a_url = geturl($rt['attachurl'],'show');
		$rt['a_url'] = is_array($a_url) ? $a_url[0] : $a_url;
		if(strpos($rt['content'],"[post]") !== false && strpos($rt['content'],"[/post]") !== false || strpos($rt['content'],"[hide") !== false && strpos($rt['content'],"[/hide]") !== false || strpos($rt['content'],"[sell") !== false && strpos($rt['content'],"[/sell]") !== false){
