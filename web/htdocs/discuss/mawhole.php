...
if (empty($_POST['step']) && is_array($tidarray)){
	...
	if($tids){
		$query = $db->query("SELECT fid,tid,author,authorid,postdate,subject,anonymous FROM pw_threads WHERE tid IN($tids)");
...
if ($action == "type"){
	...
	if (empty($_POST['step'])){
		...
		while($rt=$db->fetch_array($query)){
			...
			if($pollid){
				$rs        = $db->get_one("SELECT voteopts,state,modifiable,previewable,timelimit FROM pw_polls WHERE pollid='$pollid'");
				Add_S($rs);
				$db->update("INSERT INTO pw_polls (voteopts,state,modifiable,previewable,timelimit) VALUES('$rs[voteopts]','$rs[state]','$rs[modifiable]','$rs[previewable]','$rs[timelimit]')");
				$newpollid = $db->insert_id();
			} else{
				$newpollid = 0;
			}
			$db->update("INSERT INTO pw_threads (fid,icon,titlefont,author,authorid,subject,ifcheck, postdate,lastpost,lastposter,hits,replies,topped,locked,digest,ifupload,pollid,ifmail,ifshield,ifsale,anonymous) VALUES('$to_id','$icon','$titlefont','$author','$authorid','$subject','$ifcheck','$postdate','$lastpost','$lastposter','$hits','$replies','$topped','$locked','$digest','$ifupload','$newpollid','$ifmail','$ifshield','$ifsale','$anonymous')");
			$newtid = $db->insert_id();
			$aid    = str_replace("'","\'",$aid);
			...
			while($rt=$db->fetch_array($query2)){
				Add_S($rt);
				@extract($rt);
				$db->update("INSERT INTO pw_posts (fid,tid,aid,author,authorid,icon,postdate,subject,userip,ifsign,alterinfo,remindinfo,ipfrom,ifconvert,ifcheck,content,ifshield,anonymous) VALUES ('$to_id','$newtid','$aid','$author','$authorid','$icon','$postdate','$subject','$userip','$ifsign','$alterinfo','$remindinfo','$ipfrom','$ifconvert','$ifcheck','$content','$ifshield','$anonymous')");
			}
		}
		foreach($msgdb as $key=>$val){
			writenewmsg($val,1);
	...
	if(empty($_POST['step'])){
		$fromdb=$db->get_one("SELECT * FROM pw_threads t LEFT JOIN pw_tmsgs tm USING(tid) WHERE t.tid='$seltid'");
		!$fromdb && Showmsg('illegal_tid');
		if($fromdb['topped'] || $fromdb['ifreward'] || $fromdb['digest'] || $fromdb['pollid'] || $fromdb['buy'] || $fromdb['toolinfo'] || $fromdb['activeid']){
			Showmsg('unite_limit');
		}
		require_once PrintEot('mawhole');footer();
	} else{
		...
		$db->update("INSERT INTO pw_posts (fid,tid,aid,author,authorid,icon,postdate,subject,userip,ifsign,alterinfo,ipfrom,ifconvert,ifcheck,content,ifmark,ifshield) VALUES ('$fid','$totid','$fromdb[aid]','$fromdb[author]','$fromdb[authorid]','$fromdb[icon]','$fromdb[postdate]','$fromdb[subject]','$fromdb[userip]','$fromdb[ifsign]','$fromdb[alterinfo]','$fromdb[ipfrom]','$fromdb[ifconvert]','$fromdb[ifcheck]','$fromdb[content]','$fromdb[ifmark]','$fromdb[ifshield]')");
		$pid=$db->insert_id();
