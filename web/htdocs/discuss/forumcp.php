...
if(!$action){	
	...
	if($type=='notice'){
		...
		$rt=$db->get_one("SELECT COUNT(*) AS count FROM pw_threads WHERE fid='$fid' AND ifreward='1' AND $timestamp-postdate>'$rewtime'*86400 $sql");
		$sum=$rt['count'];
		$numofpage=ceil($sum/$db_perpage);
		$pages=numofpage($sum,$page,$numofpage,"forumcp.php?action=edit&type=reward&fid=$fid&$url_a");

		$threaddb=array();
		$query=$db->query("SELECT tid,fid,subject,author,authorid,ifreward,rewardinfo,postdate FROM pw_threads WHERE fid='$fid' AND ifreward='1' AND $timestamp-postdate>'$rewtime'*86400 $sql ORDER BY postdate $limit");
		while($rt=$db->fetch_array($query)){
...
function updatecache_i($fid){
	...
	if($notice){
		$notice['content']=convert($notice['content'],$db_windpost);
		$notice=db_cv($notice);
		$db->pw_update(
