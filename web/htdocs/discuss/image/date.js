...
function GetDate(idname){
    var sDate;  
    if (event.srcElement.tagName == "TD"){
        if (event.srcElement.innerText != ""){ 
            sDate = get_object('Year').value + "-" + get_object('Month').value + "-" + event.srcElement.innerText;
...
function ShowCalendar(idname){
	...
	table+="</select>";    
    table+="</td>";    
    table+="<td style='border-bottom:1px solid #0066FF; background-color:#84AACE; font-weight:bold; font-family:Wingdings 2,Wingdings,Webdings; font-size:16px; padding-top:2px; color:#4477FF; cursor:hand' align='center' title='關閉' onClick='javascript:HiddenCalendar()'>S</td>";
    table+="</tr>";     
    ...
    for(i = 0;i < weeks.length;i++){
        table+="<td align='center' style='font-size:12px;background-color:#E5E8EA'>" + weeks[i] + "</td>"; 
	}
	table+="</tr>";
    ...
    for(intWeeks = 0;intWeeks < 6;intWeeks++){ 
        ...
        for (intDays = 0;intDays < weeks.length;intDays++){
            table+="<td onClick='GetDate(\"" + idname + "\")' style='cursor:hand; border-right:1px solid #BBBBBB; border-bottom:1px solid #BBBBBB; color:#215DC6; font-family:Verdana; font-size:12px' align='center'></td>";
		}
        table+="</tr>"; 
    } 
