...
if($tid){
	$rt=$db->get_one("SELECT t.fid,t.tid,t.subject,t.author,t.replies,t.locked,t.postdate,t.anonymous,tm.content FROM pw_threads t LEFT JOIN pw_tmsgs tm ON tm.tid=t.tid WHERE t.tid='$tid' AND ifcheck=1");
	if($rt['locked']==2){
		wap_msg('read_locked');
	}
	if(!$rt){
	...
	$posts='';
	$query=$db->query("SELECT subject,author,content,postdate,anonymous FROM pw_posts WHERE tid='$rt[tid]' AND ifcheck=1 ORDER BY postdate $limit");
