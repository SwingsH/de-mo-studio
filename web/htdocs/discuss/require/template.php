...
if($tid){
	...
	if($foruminfo['allowvisit']){
		...
		$usehtm=0;
	}elseif($read['ifreward'] || $read['activeid']){
		$usehtm=0;
...
function htmread($read,$start_limit){
	global $tpc_author,$path,$count,$timestamp,$db_onlinetime,$db_bbsurl,$attachdir,$attachpath,$gp_allowloadrvrc,$tablecolor,$readcolorone,$readcolortwo,$lpic,$ltitle,$imgpath,$db_ipfrom,$db_showonline,$stylepath,$db_windpost,$db_windpic,$fid,$tid,$attachper,$attachments,$aids,$db_signwindcode,$md_ifopen,$_MEDALDB,$db_shield;
		...
		if(is_array($attachs)){
			foreach($attachs as $at){
				if($at['type']=='img' && $at['needrvrc']==0){
					...
					if($a_url=='imgurl'){
						$read['picurl'][$at['aid']]=$attachments[$at['aid']]="<a href=\"$path/job.php?action=showimg&tid={$tid}&pid={$pid}&fid={$fid}&aid={$at[aid]}&verify=".md5("showimg{$tid}{$pid}{$fid}{$at[aid]}{$GLOBALS[db_hash]}")."\" target=\"_blank\">$at[name]</a>";
					}else{
