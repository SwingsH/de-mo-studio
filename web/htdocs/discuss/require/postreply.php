...
$tpcarray = $db->get_one("SELECT t.fid,t.locked,t.ifcheck,t.author,t.authorid,t.postdate,t.ifmail,t.pollid ,t.subject,t.type,t.ifreward,t.rewardinfo,t.ifshield,t.anonymous $S_sql FROM pw_threads t $J_sql WHERE t.tid='$tid'");
...
/****** 悬赏 ******/
$ifreward=0;
if($tpcarray['ifreward']=='1' && $winduid!=$tpcarray['authorid']){
...
if(!$_POST['step']){
	...
	*/
	...
	$db->update("INSERT INTO pw_posts (fid,tid,aid,author,authorid,icon,postdate,subject,userip,ifsign,ipfrom,ifconvert,ifcheck,content,ifreward,anonymous) VALUES ('$fid','$tid','$attachs','".addslashes($windid)."','$winddb[uid]','$atc_iconid','$timestamp','$atc_title', '$onlineip','$atc_usesign','$ipfrom','$ifconvert','$ifcheck','$atc_content','$ifreward','$anonymous')");
	$pid = $db->insert_id();
	if($ifcheck==1){
