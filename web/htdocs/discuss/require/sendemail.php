...
Class Mailconfig {
	...
	function GetMax($maildomain){
		$header=pack("H*","000101000001000000000000");//查询头
		$end=pack("H*","00000f0001");//查询结尾
		...
		if (!$fp) {
		   ...
		   if(array_shift($q) !=0x0001){//与提交的序列号对应
			   return false;
		   }

		   if((array_shift($q) & 0x800F) !=0x8000){//审查是否为anwer,是否无错误
				return false;
		   }

		   if(array_shift($q) !=0x0001){//问题数量,应该为1
				return false;
		   }
		   ...
		   $aunum=array_shift($q);//授权记录数
		   $aanum=array_shift($q);//附加记录数
		   
		   if(fread($fp,strlen($ques)+5) !== $ques.$end){//合并问题到缓存
			   return false;
		   }
		   $data .= $ques.$end;//添加数据到数据缓存
		   //逐条读取答案数据
		   $rs=array();
		   for($i=0;$i<$anum;$i++){
			   $mxanwer=array();
			   $tdata=fread($fp,1);//读取节点长度
			   $tmx=array();
			   $tna=array();
			   $tpre=65535;
			   $compresspos=-1;//指针最后操作位置
			   for($j=0;$j<32;$j++){//读取第$i+1个答案的Name字段最大32级子域
					$tq=array_shift(unpack("C",$tdata));//解析节点长度
					if(($tq & 192)==192){//判断节点是否为指针，为指针则进入搜索操作
						if($compresspos<0){//压缩数据中的压缩数据即多次指针跳转
							$tdata .=fread($fp,1);
							$data .=$tdata;
						}else{
					...
					if($tq==0){
						break;//到达压缩数据尾
					}elseif($compresspos>0){//有数据压缩处理则从相关指针位读取
						$tna[]=array_shift(unpack("a*",substr($data,$compresspos+1,$tq)));
						$compresspos +=$tq+1;//后移指针位（包含一位节点数据位）
						$tdata=substr($data,$compresspos,1);
					}else{
						$tdata=fread($fp,$tq);
						$data .=$tdata;
			   ...
			   $tdata=fread($fp,10);//读取答案的type,class,ttl,data length,preference
			   $data .=$tdata;
			   $tq=unpack("n*",$tdata);
			   $tdata=fread($fp,$tq[5]);//读取答案的整个资源数据
			   $data .=$tdata;
			   $tttl=array_shift(unpack("n*",$tq[4]));
			   $tna=implode($tna,".");
			   if($tq[1]===15 && $tq[2]===1){
				   $tpref=array_shift(unpack("n*",substr($tdata,0,2)));//解析preference值
		   ...
		   //print_r($rs);
		   //$mxs=array();
		   arsort($rs,SORT_ASC);//按照preference值重新排序
		   foreach($rs['mx'] as $key=>$values){
			   arsort($values,SORT_ASC);//按照preference值重新排序
			   //array_merge($mxs,$values);
