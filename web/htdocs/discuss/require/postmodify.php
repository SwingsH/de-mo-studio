...
if($article==0){
	$tpcdb=$db->get_one("SELECT t.tid,t.fid AS tfid,t.author,t.authorid,t.icon,t.locked,t.postdate,t.subject,t.type,t.ifcheck,t.pollid,t.activeid,t.anonymous,tm.content,tm.aid,tm.ifsign FROM pw_threads t LEFT JOIN pw_tmsgs tm ON tm.tid=t.tid WHERE t.tid='$tid'");
...
if ($article == 0 && $pollid){
	...
	@extract($db->get_one("SELECT voteopts $sqladd FROM pw_polls WHERE pollid='$pollid'"));
	$votearray = unserialize($voteopts); 
}elseif($article == 0 && $activeid && $forumset['allowactive'] && !$step){
	$act=$db->get_one("SELECT * FROM pw_activity WHERE id='$activeid'");
	$act && $active=1;
...
if (!$_POST['step']){
	...
	if ($article == 0 && $_POST['vote']){
		if($_G['modifyvote'] && is_array($votearray) && is_array($vt_selarray)){
			...
			$db->update("UPDATE pw_polls SET voteopts='$voteopts',state='$vote_close',modifiable='$modifiable',previewable='$previewable',timelimit='$timelimit' WHERE pollid='$pollid'");
		}elseif($vote_close){
			$db->update("UPDATE pw_polls SET state='1' WHERE pollid='$pollid'");
		}
	}elseif($active && $forumset['allowactive'] && $activeid){
		...
		$db->update("UPDATE pw_activity SET subject='$active_subject',starttime='$active_starttime',endtime='$active_endtime',location='$active_location',num='$active_num',sexneed='$active_sex',costs='$active_costs',deadline='$active_deadline' WHERE id='$activeid'");
	}
	/**
	* 附件修改
	*/
		...
		foreach ($oldattach as $key=>$value){
			if (!@in_array($key,$keep)){
				...
				if($a_url[1]=='Local'){
					...
					require_once(D_P.'require/ftp.php');
					$ftp->delete($value['attachurl']);
				}			
				$db->update("DELETE FROM pw_attachs WHERE aid='$key'");
				unset($oldattach[$key]);
	...
	$sqladd = '';
	if($forumset['anonymous'] && $_G['anonymous'] && $anonymous != $pre_anonymous){ // 匿名帖切换，更新最后回复人
		$ay=$db->get_one("SELECT pid FROM pw_posts WHERE tid='$tid' ORDER BY postdate DESC LIMIT 1");
		if(!$ay && $article==0 || $pid && $pid==$ay['pid']){
