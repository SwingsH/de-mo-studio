...
function lastinfo($fid,$allowhtm=0,$type='',$sys_type=''){
	global $db,$R_url,$htmdir,$foruminfo,$tid,$windid,$timestamp,$atc_title,$t_date,$replytitle,$anonymous,$db_anonymousname;
	if($type == 'new'){
		$rt['tid']      = $tid;
	...
	$anonymous && $author = $db_anonymousname;
...
function jscv($code){
	$code = preg_replace('/[\s]on[\w]+=[^"]*"[^"]*"/is',"",$code);
	$code = preg_replace("/[\s]on[\w]+=[^']*'[^']*'/is","",$code);
	$code = preg_replace("/[\s]on[\w]+=[^\s]*/is","",$code);
