...
if (!$job) {
	...
	$cnclass='';
	while($rt=$db->fetch_array($query)){
		$cnclass .= "<option value=\"$rt[cid]\">$rt[cname]</option>";
	}
	$sql='';
	if($classid){
		$cnclass=str_replace("<option value=\"$classid\">","<option value=\"$classid\" selected>",$cnclass);
		$sql="WHERE c.classid='$classid'";
	}
	$rt    = $db->get_one("SELECT COUNT(*) AS sum FROM pw_colonys c $sql");
	$pages = numofpage($rt['sum'],$page,ceil($rt['sum']/$db_perpage),"$basename&");

	$colonydb = array();
	$query = $db->query("SELECT c.*,cl.cname AS classname FROM pw_colonys c LEFT JOIN pw_cnclass cl ON cl.cid=c.classid $sql ORDER BY createtime DESC $limit");
	while($rt = $db->fetch_array($query)){
